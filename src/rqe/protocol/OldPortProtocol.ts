...
import { UseConfig, ServeConfig, addSchemaUpdateListener, Message, ConnectReply } from './ProtocolCommon'
    ...
    start() {
        ...
        if (config.use) {
            ...
            if (config.use.expectedApi) {
                ...
                this.premountModule = this.graph.mount(
                    config.use.expectedApi.map((point: LooseMountPointSpec) => {
